...
# Expected shape: {"status":"ok","version":"1.1.7"}
```
...
`ghcr.io/itoms-hub/bearbiz:1.1.7` image by default; it does not build from a
...
The stack pins image release `1.1.7`, sets `POSTGRES_HOST=db`, and the internal
...
`v1.1.7` as `1.1.7` (plus safe semver tags and SHA). Deploy the verified version tag from the compose file for repeatable releases;

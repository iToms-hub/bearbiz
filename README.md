...
# Expected shape: {"status":"ok","version":"0.9.9"}
```
...
`ghcr.io/itoms-hub/bearbiz:0.9.9` image by default; it does not build from a
...
The stack pins image release `0.9.9`, sets `POSTGRES_HOST=db`, and the internal
...
`v0.9.9` as `0.9.9` (plus safe semver tags and SHA). Deploy the verified version tag from the compose file for repeatable releases;

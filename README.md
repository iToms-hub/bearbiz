...
# Expected shape: {"status":"ok","version":"1.1.5"}
```
...
`ghcr.io/itoms-hub/bearbiz:1.1.5` image by default; it does not build from a
...
The stack pins image release `1.1.5`, sets `POSTGRES_HOST=db`, and the internal
...
`v1.1.5` as `1.1.5` (plus safe semver tags and SHA). Deploy the verified version tag from the compose file for repeatable releases;

...
# Expected shape: {"status":"ok","version":"1.1.0"}
```
...
`ghcr.io/itoms-hub/bearbiz:1.1.0` image by default; it does not build from a
...
The stack pins image release `1.1.0`, sets `POSTGRES_HOST=db`, and the internal
...
`v1.1.0` as `1.1.0` (plus safe semver tags and SHA). Deploy the verified version tag from the compose file for repeatable releases;

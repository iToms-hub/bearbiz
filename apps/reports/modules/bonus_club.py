...
class BonusClubReport:
    slug = "bonus_club"
    display_name = "Bonus Club"
    parse_version = 1

    _ASSOCIATE_RE = re.compile(r"^\d{7}$")
    _ASSOCIATE_FLAT_RE = re.compile(
        r"^(?P<associate>\d{7})\s+(?P<name>.+?)\s+(?P<total>\d[\d,]*)\s+(?P<club>\d[\d,]*)\s+(?P<pct>[\d.,]+%)$"
    )
    _TOTALS_FLAT_RE = re.compile(r"^Totals\s+(?P<total>\d[\d,]*)\s+(?P<club>\d[\d,]*)\s+(?P<pct>[\d.,]+%)$")
    _HEADER_LABELS = {
    ...
    _FOOTER_DATE_RE = re.compile(
        r"^(?P<weekday>Sunday|Monday|Tuesday|Wednesday|Thursday|Friday|Saturday),\s+(?P<month>[A-Za-z]+)\s+(?P<day>\d{1,2}),\s+(?P<year>\d{4})$"
    )
    _VISIBLE_HEADERS = [
        "Associate #",
        "Name",
    ...
    def parse(self, raw_text: str) -> ParsedReport:
        ...
        payload = {
            ...
            "raw_text": raw_text,
            "report_title": "Bonus Club Capture Report",
    ...
    def _parse_flat_associate_row(cls, line: str) -> dict[str, Any] | None:
        ...
        if match is None:
            return None
        values = [match.group("total"), match.group("club"), match.group("pct")]
    ...
    def _build_row(
        ...
        *,
        ...
        club_transactions = cls._to_int(values[1])
        capture_rate = cls._to_percent(values[2])
        metrics = {
            "associate_number": associate_number,
            "name": name,
            "source_label": source_label,
            "total_transactions": total_transactions,
            "transactions_with_club": club_transactions,
        ...
        return {
            ...
            "visible_values": [
                ...
                cls._format_number(club_transactions),
                cls._format_percent(capture_rate),
            ],
            "viewer": {
                "values": [
                    associate_number,
    ...
    def _viewer_row(cls, row: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            "values": [
                ...
                cls._format_number(metrics_dict.get("transactions_with_club")),
                cls._format_percent(metrics_dict.get("capture_rate")),
            ]
        }
    ...
    def _is_footer_line(cls, line: str) -> bool:
        lowered = line.lower().strip()
        return lowered.startswith(("store number", "from date", "to date", "page ", "bonus club capture report")) or bool(cls._FOOTER_DATE_RE.match(line))


def load_bonus_club_report() -> BonusClubReport:
    report = BonusClubReport()
    register(report)
    return report


load_bonus_club_report()

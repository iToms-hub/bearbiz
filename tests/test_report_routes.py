...
def test_weekly_sales_upload_history_and_download_routes(client: Client, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with override_settings(MEDIA_ROOT=media_root):
        ...
        assert '© 2026 · coded by Claire · itoms.org · v1.2.0' in history_html
...
def test_dashboard_shows_last_week_store_bonus_club_and_gift_cards_metrics(client: Client, tmp_path: Path) -> None:
    ...
    assert "dashboard-card-payroll" in html
    assert "No Payroll data available." in html
    assert "No persisted Bonus Club data is available." not in html
    assert "No persisted Gift Cards data is available." not in html
